...
uint16_t Adafruit_ADS1015::readADC_SingleEnded(uint8_t channel) {
  ...
  uint16_t result = 0;  // GoRoNb
  result = readRegister(m_i2cAddress, ADS1015_REG_POINTER_CONVERT) >> m_bitShift;  // GoRoNb
  ...
  // delay(m_conversionDelay);  // GoRoNb

  // Read the conversion results
  // Shift 12-bit results right 4 bits for the ADS1015
  return result;  // GoRoNb
}
...
int16_t Adafruit_ADS1015::readADC_Differential_0_1() {
  ...
  // Read the conversion results
  uint16_t result = readRegister(m_i2cAddress, ADS1015_REG_POINTER_CONVERT) >> m_bitShift;
  if (m_bitShift != 0)
  {
    // Shift 12-bit results right 4 bits for the ADS1015,
