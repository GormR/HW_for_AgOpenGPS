  ...
  */
  ...
  #define DIR 2  // direction pin for serial communication: high = send
  ...
  {
    pinMode(DIR, OUTPUT);
    digitalWrite(DIR, LOW);  // default = receive
    ...
    uint8_t error;
    digitalWrite(DIR, HIGH);  // start sending
    Serial.println("Checking for CMPS14");
    delay(1);                 // 0.26ms per byte
    digitalWrite(DIR, LOW);   // default = receive
